namespace Devity.Extensions
{
    public static class DateTimeExtensions
    {
        ...
        public static TimeSpan GetUntilEndOfDay(this DateTime currentTime)
            => TimeSpan.FromSeconds(SECONDS_IN_DAY - ((currentTime.Hour * SECONDS_IN_HOUR) + (currentTime.Minute * SECONDS_IN_MINUTE) + currentTime.Second));
        ...
        public static TimeSpan GetUntilEndOfMonth(this DateTime currentTime)
            => TimeSpan.FromSeconds(((DateTime.DaysInMonth(currentTime.Year, currentTime.Month) + 1) * SECONDS_IN_DAY) - ((currentTime.Day * SECONDS_IN_DAY) + (currentTime.Hour * SECONDS_IN_HOUR) + (currentTime.Minute * SECONDS_IN_MINUTE) + currentTime.Second));
